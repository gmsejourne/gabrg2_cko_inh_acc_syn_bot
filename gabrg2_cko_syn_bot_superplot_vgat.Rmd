---
title: "gabrg2_cko_syn_bot_superplot_vgat"
...
counts <- data.frame(animal = p42_counts_decoded$Animal,
                     ...
                     vgat_count = p42_counts_decoded$`Red Puncta Count`)
...
library(FSA)
  ...
  Summarize(vgat_count ~ layer + genotype,
          data=counts,
          digits=3)
```
...
```{r}
library(car)
model = lm(vgat_count ~ layer + genotype + layer:genotype,
           data = counts)
summary(model)
...
ReplicateAverages <- counts %>% group_by(genotype, animal, layer) %>%
  summarise(vgat_count = mean(vgat_count))
ReplicateAverages

# Gives the p-value for the t-Test of variable 1 and 2
ttest1 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 1,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest1p <- ttest1[["p.value"]]
ttest1
ttest1p

ttest2 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 2,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest2p <- ttest2[["p.value"]]
ttest2
ttest2p

ttest3 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 3,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest3p <- ttest3[["p.value"]]
ttest3
ttest3p

ttest4 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 4,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest4p <- ttest4[["p.value"]]
ttest4
ttest4p

ttest5 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 5,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest5p <- ttest5[["p.value"]]
ttest5
ttest5p

ttest6 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 6,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest6p <- ttest6[["p.value"]]
ttest6
ttest6p

ttest7 <- t.test(vgat_count  ~ genotype, data = counts[counts$layer == 7,], alternative = "two.sided", var.equal = TRUE, conf.level = 0.95)
ttest7p <- ttest7[["p.value"]]
ttest7
ttest7p

# Calculates total averages
TotalAverages <- ReplicateAverages %>% group_by(genotype, layer) %>%  summarise(vgat_count = mean(vgat_count))
TotalAverages
...
superplot_p42 <- ggplot(counts, aes(x=genotype,y=vgat_count,color=factor(animal))) +
  ...
  theme(
    ...
    ggtitle ("VGAT puncta") + xlab("") + ylab("VGAT counts") 
ppi = 300
png(filename = "Gabrg2_cko_v_ctrl_superplot_p42_vgat.png", width = 8*ppi, height = 8*ppi, res = ppi)
  superplot_p42
  dev.off()
superplot_p42
```
